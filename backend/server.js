...
app.put('/api/certifications/:certNumber', async (req, res) => {
  ...
  const tableMap = {
    'Singlas Electrical': 'Singlas_Electrical',
    'Singlas Temperature': 'Singlas_Temperature',
    'Singlas Pressure': 'Singlas_Pressure',
    'Electrical': 'Non_Singlas_Electrical',
    'Temperature': 'Non_Singlas_Temperature',
    'Pressure': 'Non_Singlas_Pressure',
    'Singlas On-site': 'Singlas_On_Site',
    'Non Singlas On-Site': 'Non_Singlas_On_Site',
  };

  const tableName = tableMap[certificateType];
  if (!tableName) return res.status(400).json({ error: 'Invalid certificate type' });
